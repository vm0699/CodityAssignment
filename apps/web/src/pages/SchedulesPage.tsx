...
export default function SchedulesPage() {
  ...
  async function remove(schedule: Schedule) {
    if (!window.confirm(`Delete schedule "${schedule.name}"? Already-created jobs are kept.`)) return;
    try {
  ...
  return (
    <div className="space-y-5 p-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-slate-100">Recurring schedules</h1>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {schedules.data.map((s) => {
                ...
                return (
                  <tr key={s.id} className="border-b border-surface-800 last:border-0">
                    <td className="px-4 py-3 font-medium text-slate-200">{s.name}</td>
                    <td className="px-3 py-3">
                      <code className="rounded bg-surface-800 px-1.5 py-0.5 font-mono text-xs text-sky-300">{s.cron_expression}</code>
                      <div className="mt-0.5 text-xs text-slate-600">{s.cron_description !== s.cron_expression ? s.cron_description : ''} {s.timezone !== 'UTC' ? `(${s.timezone})` : ''}</div>
                    </td>
                    <td className="px-3 py-3 text-slate-400">{queue?.name ?? '—'} · {s.job_type}</td>
                    <td className="px-3 py-3">
