...
export default function DlqPage() {
  ...
  async function purge(entry: DlqEntry) {
    if (!window.confirm('Permanently delete this job and all of its history?')) return;
    setError(null);
    try {
              ...
              {entries.data.map((entry) => (
                <tr key={entry.id}
                  ...
                  <td className="px-4 py-3">
                    <div className="font-medium text-slate-200">{entry.job_type}</div>
                    <div className="font-mono text-xs text-slate-600">{shortId(entry.job_id)}</div>
                  </td>
                  <td className="px-3 py-3 text-slate-400">{entry.queue_name}</td>
                  <td className="px-3 py-3 text-right font-mono">{entry.attempts_made}</td>
                  <td className="px-3 py-3 text-slate-500">
                    {timeAgo(entry.moved_at)}
                    {entry.requeued_at && <span className="ml-2 rounded bg-sky-500/15 px-1.5 py-0.5 text-xs text-sky-300">requeued</span>}
                  </td>
                  <td className="max-w-[280px] truncate px-3 py-3 text-red-300/80" title={entry.error ?? ''}>{entry.error}</td>
                  <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                    {!entry.requeued_at && (
                      <div className="flex justify-end gap-1">
                        <button title="Requeue" onClick={() => void requeue(entry)}
                          className="rounded p-1.5 text-slate-400 hover:bg-surface-700 hover:text-emerald-300">
                          <RefreshCw size={15} />
                        </button>
          ...
          <div className="space-y-4 text-sm">
            ...
            {inspecting.failure_summary && (
              <div className="rounded-lg border border-red-500/30 bg-red-500/10 px-3 py-2">
                ...
                <p className="text-red-200">{inspecting.failure_summary}</p>
              </div>
            )}
            <div>
              <h4 className="mb-1.5 text-xs font-semibold uppercase tracking-wider text-slate-500">Last error</h4>
              <pre className="max-h-32 overflow-auto whitespace-pre-wrap rounded-lg bg-surface-950 p-3 font-mono text-xs text-red-300/90">
                {inspecting.error ?? '—'}
              </pre>
            </div>
            <div>
